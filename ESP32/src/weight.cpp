...
const int gain = 64;  // Reading values with 64 bits (could be 128 too)
...
float correctionAlgo(float realValue)
{
  ...
  // Round algorithm //
  if (realValueDecim < 0.4)
  {
    correctedValue = long(realValueInt);
  }
  else if (realValueDecim <= 0.7)
  {
    correctedValue = long(realValueInt) + 0.5;
  }
  else
  {
    correctedValue = long(realValueInt) + 1;
  }
